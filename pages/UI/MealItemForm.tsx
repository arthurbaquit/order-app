...
export const MealItemForm = ({onSubmitHandler}:{onSubmitHandler: (amount: number) => void}) => {
    ...
    const onItemQntdHandler = (event:ChangeEvent<HTMLInputElement>) =>{
        if(+event.target.value>0 )
            setItemQtd(+event.target.value)
    }

    return(
        <form className={styles.form} onSubmit={onSubmit}>
                <Input label={'Amount'} id={'key'} type={'number'} onChange={onItemQntdHandler} value={itemQtd}/>
